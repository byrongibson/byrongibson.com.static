...
!function(global) {
    ...
    function newContext(contextName) {
        ...
        function normalize(name, baseName, applyMap) {
            ...
            pkgConfig = getOwn(config.pkgs, pkgName = name[0]), name = name.join("/"), pkgConfig && name === pkgName + "/" + pkgConfig.main && (name = pkgName)) : 0 === name.indexOf("./") && (// No baseName, so this is ID is resolved relative
            // to baseUrl, pull off the leading dot.
            name = name.substring(2))), applyMap && map && (baseParts || starMap)) {
        ...
        function hasPathFallback(id) {
            ...
            return pathConfig && isArray(pathConfig) && pathConfig.length > 1 ? (//Pop off the first array value, since it failed, and
            //retry
        ...
        function makeModuleMap(name, parentModuleMap, isNormalized, applyMap) {
            ...
            pluginModule = getOwn(defined, prefix)), //Account for relative paths if there is a base name.
            name && (prefix ? //Plugin is loaded, use its normalize method.
            normalizedName = pluginModule && pluginModule.normalize ? pluginModule.normalize(name, function(name) {
                return normalize(name, parentName, applyMap);
            }) : normalize(name, parentName, applyMap) : (//A regular module.
            ...
            isNormalized = !0, url = context.nameToUrl(normalizedName))), //If the id is a plugin id that cannot be determined if it needs
        ...
        function checkLoaded() {
            ...
            expired = waitInterval && context.startTime + waitInterval < new Date().getTime(), noLoads = [], reqCalls = [], stillLoading = !1, needCycleCheck = !0;
            ...
            if (!inCheckLoaded) {
                ...
                eachProp(enabledRegistry, function(mod) {
                    ...
                    !mod.error)) //If the module should be executed, and it has not
                ...
                expired && !usingPathFallback || !stillLoading || //Something is still waiting to load. Wait for it, but only
                //if a timeout is not already in effect.
                !isBrowser && !isWebWorker || checkLoadedTimeoutId || (checkLoadedTimeoutId = setTimeout(function() {
        ...
        return handlers = {
                        ...
                        if (this.error) this.emit("error", this.error); else if (!this.defining) {
                            ...
                            this.defining = !0, this.depCount < 1 && !this.defined) {
                                if (isFunction(factory)) {
                                    ...
                                    exports = this.exports)), err) return err.requireMap = this.map, err.requireModules = this.map.isDefine ? [ this.map.id ] : null, 
            ...
            callPlugin: function() {
                ...
                this.depMaps.push(pluginMap), on(pluginMap, "defined", bind(this, function(plugin) {
                    ...
                    })), normalizedMod = getOwn(registry, normalizedMap.id), normalizedMod && (//Mark this as a dependency for this plugin, so it
                    //can be traced for cycles.
                    this.depMaps.push(normalizedMap), this.events.error && normalizedMod.on("error", bind(this, function(err) {
                        this.emit("error", err);
                    ...
                    load.fromText = bind(this, function(text, textAlt) {
                        ...
                        textAlt && (text = textAlt), //Turn off interactive script matching for IE for any define
                        ...
                        hasInteractive && (useInteractive = !0), //Mark this as a dependency for the plugin
                        //resource
            ...
            enable: function() {
                enabledRegistry[this.map.id] = this, this.enabled = !0, //Set flag mentioning that the module is enabling,
            ...
            makeRequire: function(relMap, options) {
                function localRequire(deps, callback, errback) {
                    ...
                    "string" == typeof deps ? isFunction(callback) ? onError(makeError("requireargs", "Invalid require call"), errback) : //If require|exports|module are requested, get the
                    //value for them from the special handlers. Caveat:
                    //this only works while module is being defined.
                    relMap && hasProp(handlers, deps) ? handlers[deps](registry[relMap.id]) : //Synchronous access to one module. If require.get is
                    //available (as in the Node adapter), prefer that.
                    req.get ? req.get(context, deps, relMap, localRequire) : (//Normalize module name, if it contains . or ..
                    map = makeModuleMap(deps, relMap, !1, !0), id = map.id, hasProp(defined, id) ? defined[id] : onError(makeError("notloaded", 'Module name "' + id + '" has not been loaded yet for context: ' + contextName + (relMap ? "" : ". Use require([])")))) : (//Grab defines waiting in the global queue.
                ...
                relMap || (localRequire.undef = function(id) {
                    ...
                    mod && (//Hold on to listeners in case the
                    //module will be attempted to be reloaded
                    //using a different config.
                    mod.events.defined && (undefEvents[id] = mod.events), cleanRegistry(id));
                }), localRequire;
            },
            /**
            ...
            completeLoad: function(moduleName) {
                ...
                mod = getOwn(registry, moduleName), !found && !hasProp(defined, moduleName) && mod && !mod.inited) {
     ...
     */
        req = requirejs = function(deps, callback, errback, optional) {
            ...
            return isArray(deps) || "string" == typeof deps || (// deps is a config object
            config = deps, isArray(callback) ? (// Adjust args if there are dependencies
            deps = callback, callback = errback, errback = optional) : deps = []), config && config.context && (contextName = config.context), 
     ...
     */
        req.load = function(context, moduleName, url) {
            ...
            return node = req.createNode(config, moduleName, url), node.setAttribute("data-requirecontext", context.contextName), 
            ...
            node.addEventListener("error", context.onScriptError, !1)) : (//Probably IE. IE (at least 6-8) do not fire
            //script onload right after executing the script, so
            //we cannot tie the anonymous define call to a name.
            //However, IE reports the script as being in 'interactive'
            //readyState at the time of the define call.
            useInteractive = !0, node.attachEvent("onreadystatechange", context.onScriptLoad)), 
            node.src = url, //For some cache cases in IE 6-8, the script executes before the end
            //of the appendChild execution, so to tie an anonymous define
            //call to the module name (which is stored on the node), hold on
            //to a reference to this node, but clear after the DOM insertion.
            currentlyAddingScript = node, baseElement ? head.insertBefore(node, baseElement) : head.appendChild(node), 
        ...
        eachReverse(scripts(), function(script) {
            ...
            return head || (head = script.parentNode), //Look for a data-main attribute to set main script for the page
            //to load. If it is there, the path to data main becomes the
            //baseUrl, if it is not already set.
            dataMain = script.getAttribute("data-main"), dataMain ? (//Preserve dataMain in case it is a path (i.e. contains '?')
     ...
     */
        define = function(name, deps, callback) {
            ...
            isArray(deps) || (callback = deps, deps = null), //If no name, and callback is a function, then figure out if it a
            //CommonJS thing with dependencies.
            !deps && isFunction(callback) && (deps = [], //Remove comments from the callback string,
