 ...
 */
   ...
   */
                dangerouslyReplaceNodeWithMarkup: function(oldChild, markup) {
                    invariant(ExecutionEnvironment.canUseDOM, "dangerouslyReplaceNodeWithMarkup(...): Cannot render markup in a " + "worker thread. This is likely a bug in the framework. Please report " + "immediately.");
            ...
            function recomputePluginOrdering() {
                ...
                for (var pluginName in namesToPlugins) {
                    ...
                    for (var eventName in publishedEvents) {
                        invariant(publishEventForPlugin(publishedEvents[eventName], PluginModule), "EventPluginRegistry: Failed to publish event `%s` for plugin `%s`.", eventName, pluginName);
                    }
                }
            }
            /**
   ...
   */
                getPluginModuleForEvent: function(event) {
                    ...
                    if (dispatchConfig.registrationName) {
                        return EventPluginRegistry.registrationNames[dispatchConfig.registrationName] || null;
                    }
 ...
 */
            var LinkedValueMixin = {
                _assertLink: function() {
                    invariant(this.props.value == null && this.props.onChange == null, "Cannot provide a valueLink and a value or onChange event. If you " + "want to use value or onChange, you probably don't want to use " + "valueLink");
                },
                /**
 ...
 */
   ...
   */
     ...
     */
                    getDOMNode: function() {
                        invariant(this.isMounted(), "getDOMNode(): A component must be mounted to have a DOM node.");
   ...
   */
                mountImageIntoNode: function(markup, container, shouldReuseMarkup) {
                    invariant(container && (container.nodeType === ELEMENT_NODE_TYPE || container.nodeType === DOC_NODE_TYPE && ReactMount.allowFullPageRender), "mountComponentIntoNode(...): Target container is not valid.");
 ...
 */
            function mergeObjectsWithNoDuplicateKeys(one, two) {
                ...
                objMap(two, function(value, key) {
                    invariant(one[key] === undefined, "mergeObjectsWithNoDuplicateKeys(): " + "Tried to merge two objects with the same key: %s", key);
                    one[key] = value;
                });
                return one;
            }
 ...
 */
   ...
   */
                _renderValidatedComponent: function() {
                    ...
                    invariant(ReactComponent.isValidComponent(renderedComponent), "%s.render(): A valid ReactComponent must be returned.", this.constructor.displayName || "ReactCompositeComponent");
 ...
 */
            function assertValidProps(props) {
                ...
                invariant(props.children == null || props.dangerouslySetInnerHTML == null, "Can only set one of `children` or `props.dangerouslySetInnerHTML`.");
                invariant(props.style == null || typeof props.style === "object", "The `style` prop expects a mapping from style properties to values, " + "not a string.");
            }
            /**
 ...
 */
            function selectValueType(props, propName, componentName) {
                ...
                if (props.multiple) {
                    invariant(Array.isArray(props[propName]), "The `%s` prop supplied to <select> must be an array if `multiple` is " + "true.", propName);
                } else {
                    invariant(!Array.isArray(props[propName]), "The `%s` prop supplied to <select> must be a scalar value if " + "`multiple` is false.", propName);
                }
            }
            /**
 ...
 */
   ...
   */
                listenAtTopLevel: function(touchNotMouse, contentDocument) {
                    ...
                    trapBubbledEvent(topLevelTypes.topCompositionEnd, "compositionend", mountAt);
                    trapBubbledEvent(topLevelTypes.topCompositionStart, "compositionstart", mountAt);
 ...
 */
   ...
   */
                findComponentRoot: function(ancestorNode, id) {
                    ...
                    invariant(false, "findComponentRoot(..., %s): Unable to find element. This probably " + "means the DOM was unexpectedly mutated (e.g. by the browser).", id, ReactMount.getID(ancestorNode));
                },
                /**
   * React ID utilities.
   */
 ...
 */
   ...
   */
                mountComponent: function(rootID, transaction, mountDepth) {
                    ReactComponent.Mixin.mountComponent.call(this, rootID, transaction, mountDepth);
                    return "<span " + ReactMount.ATTR_NAME + '="' + rootID + '">' + escapeTextForBrowser(this.props.text) + "</span>";
                },
                /**
 ...
 */
            function enqueueUpdate(component, callback) {
                invariant(!callback || typeof callback === "function", "enqueueUpdate(...): You called `setProps`, `replaceProps`, " + "`setState`, `replaceState`, or `forceUpdate` with a callback that " + "isn't callable.");
                ensureBatchingStrategy();
 ...
 */
            function mutateHTMLNodeWithMarkup(node, markup) {
                invariant(node.tagName.toLowerCase() === "html", 'mutateHTMLNodeWithMarkup(): node must have tagName of "html", got %s', node.tagName);
                markup = markup.trim();
                invariant(markup.toLowerCase().indexOf("<html") === 0, "mutateHTMLNodeWithMarkup(): markup must start with <html");
